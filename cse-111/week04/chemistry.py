...
# From: https://byui-cse.github.io/cse111-course/lesson08/prove.html
...
def main():
    while True:
        ...
        answer = input('\n Do you want to calculate again? Please type yes or no ').lower()
        if answer == 'no':
            break
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for symbols in symbol_quantity_list:
        ...
        quantity = symbols[QUANTITY_INDEX]
        element_attribute = periodic_table_dict[symbol]
        mass = element_attribute[ATOMIC_MASS_INDEX]
...
def get_formula_name(formula, known_molecules_dict):
    ...
    keys = list(known_molecules_dict.keys())
    values = list(known_molecules_dict.values())
    

    if formula in keys:
        my_index = keys.index(formula)
        name = values[my_index]
        print(name)
    else:
